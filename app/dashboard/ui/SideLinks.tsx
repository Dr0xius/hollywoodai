...
const SideLinks = ({ link, onClick, handleSignOut }: SidebarLink) => {
  ...
  return link.cursor ? (
    <Link
      ...
      className="flex gap-2 hover:bg-blue-400/10 hover:text-blue-100 px-2 py-1 rounded-2xl items-center transition"
    >
      <span className="w-4 h-4 md:w-4 md:h-4">{link.icon}</span>
      <span className="">{displayLink}</span>
    </Link>
  ) : (
    <span className="flex gap-2 hover:bg-blue-400/10 hover:text-blue-100 px-2 py-1 rounded-2xl items-center transition cursor-not-allowed">
      <span className="w-4 h-4 md:w-4 md:h-4">{link.icon}</span>
